...
// GetRekapKader mengembalikan rekap kepatuhan TTD/MMS semua ibu hamil
// yang berada di posyandu yang sama dengan kader.
// Urutan: kehamilan aktif (TRIMESTER) dulu, baru NIFAS.
func (u *logTTDMMSUsecase) GetRekapKader(posyanduID *int32) ([]repositories.RekapIbuTTDMMS, error) {
	if posyanduID == nil {
		return nil, errors.New("posyandu_id tidak valid")
	}
	...
	// Pisah dua kelompok agar bisa diurutkan: hamil aktif dulu, NIFAS belakangan
	var resultHamil []repositories.RekapIbuTTDMMS
	var resultNifas []repositories.RekapIbuTTDMMS
	...
	for _, kehamilan := range kehamilanList {
		...
		// Untuk kehamilan aktif, cap di bulan 9 (270 hari).
		// Untuk NIFAS, cap tetap di 10 (seluruh masa kehamilan sudah berlalu).
		maxBulan := int32(9)
		if isNifas {
			maxBulan = 10
		}
		if bulanKeSaatIni > maxBulan {
			bulanKeSaatIni = maxBulan
		}
 
		// Total hari yang seharusnya sudah dijalani
		totalHariSeharusnya := int32(selisihHari + 1)
		if totalHariSeharusnya < 0 {
			totalHariSeharusnya = 0
		}
		// Batas maksimal 300 hari (10 bulan × 30 hari)
		if totalHariSeharusnya > 300 {
			totalHariSeharusnya = 300
		}
		...
		rekap := repositories.RekapIbuTTDMMS{
			HPHT:            kehamilan.HPHT.Format("2006-01-02"),
			IbuID:           kehamilan.IbuID,
			KehamilanID:     kehamilan.ID,
			NamaIbu:         namaIbu,
		...
		if isNifas {
			resultNifas = append(resultNifas, rekap)
		} else {
			resultHamil = append(resultHamil, rekap)
		}
	}

	// Gabung: hamil aktif dulu, lalu NIFAS
	result := append(resultHamil, resultNifas...)
	return result, nil
}
