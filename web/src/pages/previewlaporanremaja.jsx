...
export default function LaporanRemajaPreview() {
  ...
  const fetchPreview = async () => {
    ...
    try {
      ...
      if (filterEnabled && startDate && endDate) {
        response = await previewLaporanRemaja(startDate, endDate);
      } else {
        response = await previewLaporanRemaja();
      }
      ...
      const msg =
        err.response?.data?.message || err.message || "Gagal memuat preview data remaja";
      setError(msg);
      setData([]);
      setDynamicHeaders([]);
    } finally {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      a.href = url;
      const filename = `laporan_remaja_${
      ...
      Swal.fire({
        ...
        text:
          "Gagal mengekspor laporan remaja: " +
            ...
            <thead className="bg-gray-50 sticky top-0 z-10">
              <tr>
                ...
                {allColumns.map((col) => (
                  <th
                    key={col.key}
                    className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider whitespace-nowrap bg-gray-50"
                  >
                    {col.label}
                  </th>
                ))}
