...
const icons = {
  ...
  baby: (
    <svg
      ...
      viewBox="0 0 24 24"
    >
      <path d="M12 2a5 5 0 0 0-5 5c0 2.5 2 5 5 5s5-2.5 5-5a5 5 0 0 0-5-5z" />
      <path d="M7 13c-2 2-3 5-3 7h16c0-2-1-5-3-7" />
  ...
  school: (
    ...
    >
      ...
      <path d="M6 10v4c0 1.5 3 3 6 3s6-1.5 6-3v-4" />
    </svg>
  ),
  teen: (
    <svg
      ...
      viewBox="0 0 24 24"
    >
      <path d="M12 2a3 3 0 0 0-3 3c0 1.5 1 2.5 2 3" />
      <path d="M17 15v-2c0-2-2-4-5-4s-5 2-5 4v2" />
      <circle cx="9" cy="9" r="1" />
      <circle cx="15" cy="9" r="1" />
      <path d="M7 19h10" />
    </svg>
  ...
  adult: (
    ...
    >
      ...
      <path d="M20 22v-2a6 6 0 0 0-6-6h-4a6 6 0 0 0-6 6v2" />
  ...
  elderly: (
    ...
    >
      ...
      <path d="M18 22v-2a4 4 0 0 0-4-4h-4a4 4 0 0 0-4 4v2" />
      <path d="M8 11l-2 3 2 1" />
      <path d="M16 11l2 3-2 1" />
      <path d="M12 14v4" />
    </svg>
  ),
  imm: (
  ...
  ambulance: (
    <svg
      ...
      viewBox="0 0 24 24"
    >
      <path d="M5 13h4v4H5zm6 0h4v4h-4z" />
      <path d="M18 16h-3v-4h3a2 2 0 012 2v2a2 2 0 01-2 2z" />
      <path d="M6 16H4a2 2 0 01-2-2v-2a2 2 0 012-2h14" />
      <path d="M9 12V8a2 2 0 012-2h6a2 2 0 012 2v4" />
...
function RingkasanCard({
  ...
  return (
    ...
    >
      ...
      >
        <span>
          {cakupanLabel}: {persentase.toFixed(1)}%
        </span>
      ...
      {categories.map((cat) => {
        ...
        return (
          ...
          >
            ...
            <div style={{ fontSize: 14, fontWeight: "bold", color: cat.color }}>
              {value}
            </div>
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchIbu = async () => {
      try {
        ...
        const kehamilanList = rawData.filter(
          (item) => item.kehamilan_id && item.kehamilan_id !== 0,
        );
        const total_kehamilan = kehamilanList.length;
        const aktifList = kehamilanList.filter((item) =>
          item.status_kehamilan?.includes("TRIMESTER"),
        );
        const kehamilan_aktif = aktifList.length;

        const risikoTinggiList = kehamilanList.filter(
          (item) => normalizeRisk(item.status_risiko) === "Tinggi",
        );
        const risikoSedangList = kehamilanList.filter(
          (item) => normalizeRisk(item.status_risiko) === "Sedang",
        );
        const risikoNormalList = kehamilanList.filter(
          (item) => normalizeRisk(item.status_risiko) === "Normal",
        );

        const dusunMap = new Map();
        kehamilanList.forEach((item) => {
          const dusun =
            item.dusun && item.dusun.trim() ? item.dusun : "Tidak diketahui";
          dusunMap.set(dusun, (dusunMap.get(dusun) || 0) + 1);
        });
        const per_dusun = Array.from(dusunMap.entries()).map(
  ...
  useEffect(() => {
    const fetchKelompok = async () => {
      try {
        const [resKesehatan, resCakupan] = await Promise.all([
          getKesehatanKelompok(),
          getCakupanPemeriksaan(),
        ]);
  ...
  const getRiskData = () => {
    if (selectedKategori === "ibu-hamil") {
      ...
      const data = kesehatanKelompok?.[selectedKategori] || {
        Tinggi: 0,
        Sedang: 0,
        Normal: 0,
      };
  ...
  const getCakupanPersen = () => {
    if (selectedKategori === "ibu-hamil") {
      return statsIbu.total_kehamilan > 0
        ? (statsIbu.kehamilan_aktif / statsIbu.total_kehamilan) * 100
        : 0;
    } else {
      const dataCakupan = getCakupanByKelompok(selectedKategori);
  ...
  const kelompokList = [
    {
      ...
      color: "#d97706",
      tooltip: "Cakupan imunisasi & pemantauan tumbuh kembang balita",
    },
    {
      key: "anak",
      label: "Anak (6- 9 tahun)",
      color: "#0284c7",
      tooltip: "Pemeriksaan kesehatan anak usia sekolah",
    },
    {
      key: "remaja",
      label: "Remaja (10-18 tahun)",
      tooltip: "Kesehatan reproduksi & skrining remaja",
    },
    {
      key: "dewasa",
      label: "Dewasa (19-59 tahun)",
      tooltip: "Skrining PTM & pemeriksaan kesehatan umum",
    },
    {
      key: "lansia",
      label: "Lansia (60+ tahun)",
      tooltip: "Pemantauan penyakit kronis & kunjungan lansia",
    },
  ];
  ...
  const persentaseKehamilanAktif =
    totalKehamilan > 0 ? (kehamilanAktif / totalKehamilan) * 100 : 0;
  ...
  const ibuHamilCard = {
    ...
    tooltipText:
      "Persentase kehamilan yang masih berjalan (trimester 1-3) dari total kehamilan tercatat",
    // icon: icons.preg,
    color: "#db2777",
    // bg: "#ffe4e6",
      ...
      >
        {showOnboarding && (
          ...
          >
            ...
            <p style={{ fontSize: 13, marginBottom: 8 }}>
              • Klik batang grafik risiko untuk melihat daftar individu
              berdasarkan risiko.
            </p>
            <p style={{ fontSize: 13 }}>
              • Klik kartu ringkasan (Ibu Hamil, Balita, Anak, dll) untuk
              langsung berpindah kategori.
            </p>
            <p style={{ fontSize: 13, marginTop: 8 }}>
              • Klik nama individu untuk melihat detail pemeriksaan.
            </p>
          </div>
        )}
        ...
        <div style={{ padding: "16px 20px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div>
                <div style={{ fontSize: 14, fontWeight: 500, opacity: 0.9 }}>
                  Jadwal Layanan Posyandu Hari Ini
                </div>
                <div style={{ fontSize: 28, fontWeight: "bold" }}>
                  {todayScheduleCount} sesi
                </div>
                <div style={{ fontSize: 11, opacity: 0.7 }}>
                  Klik tombol untuk kelola jadwal
                </div>
              </div>
            </div>
            <button
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            >
              <h3 style={{ fontSize: 13, fontWeight: 600, color: "#334155" }}>
                Ringkasan Sasaran & Cakupan
              </h3>
            </div>
            <div
              ...
              tabIndex={0}
              role="region"
              aria-label="Ringkasan sasaran dan cakupan"
            >
              <div
                style={{
                  display: "grid",
          ...
          >
            ...
            >
              ...
              >
                {icons.info} <strong>Petunjuk:</strong> Klik salah satu batang
                (Tinggi/Sedang/Rendah) untuk melihat daftar individu yang masuk
                dalam kategori risiko tersebut.
              </div>
              <RiskBarChart
                data={getRiskData()}
              ...
              >
                ...
                >
                  ...
                  <div style={{ fontSize: 9, color: "#94a3b8" }}>
                    {getTotalRisiko() > 0
                      ? (
                          (getRisikoTinggiCount() / getTotalRisiko()) *
                          100
                        ).toFixed(0)
                      : 0}
                    % dari total risiko
              ...
              >
                Klik batang untuk melihat daftar individu. Warna batang
            ...
            >
              <div style={{ flexShrink: 0 }}>
                ...
                >
                  ...
                  >
                    ...
                    >
                      {selectedDusun
                        ? dusunIbuList.length
                        : selectedKategori === "ibu-hamil"
                          ? filteredIbuList.length
                          : daftarKelompok.length}{" "}
                      orang
                    </span>
                  </div>
                  {!selectedDusun && selectedKategori === "ibu-hamil" && (
                    <button
                      onClick={() =>
                        navigate(
                          `/data-ibu?risiko=${encodeURIComponent(getFilterFromRisk(selectedRiskFilter))}`,
                        )
                      }
                      style={{
                        background: "#3b82f6",
                ...
                {!selectedDusun && (
                  ...
                  >
                    <strong>Informasi:</strong> Daftar ini akan muncul setelah
                    Anda mengklik batang risiko pada grafik di sebelah kiri.
                ...
                {selectedDusun && (
                  ...
                  >
                    Klik nama ibu untuk pemeriksaan. Tombol{" "}
                    <strong>Rujuk</strong> untuk kasus perlu rujukan.
                  </p>
                )}
              </div>
                ...
                >
                  ...
                  {selectedDusun ? (
                    dusunIbuList.length > 0 ? (
                      dusunIbuList.map((r, idx) => {
                        const riskLevel = normalizeRisk(
                          r.isRujukan ? "PERLU RUJUKAN" : "NORMAL",
                        );
                        ...
                        return (
                          <div
                            ...
                            onClick={() =>
                              navigate(
                                `/data-ibu/${r.ibu_id}/pemeriksaan-rutin?kehamilan_id=${r.kehamilan_id}`,
                              )
                            }
                            style={{
                              ...
                              {r.isRujukan && (
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    navigate(
                                      `/data-ibu/${r.ibu_id}/rujukan?kehamilan_id=${r.kehamilan_id}`,
                                    );
                                  }}
                                  style={{
                                    background: "#dc2626",
                    ...
                    daftarKelompok.map((p, idx) => {
                      const riskStyle = getRiskCardStyle(
                        activeRiskKelompok || "Normal",
                      );
                      return (
                        <div
                          key={idx}
                    ...
                    >
                      Belum ada data. Klik batang pada grafik risiko terlebih
                      dahulu.
                    </div>
                  )}
                </div>
