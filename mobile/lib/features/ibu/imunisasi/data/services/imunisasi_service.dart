...
//           'nama_anak': localData.first['nama_anak'],
...
class ImunisasiService {
  ...
  Future<List<ImunisasiModel>> getJadwalImunisasi() async {
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) {
      final msg = body['message'];
      throw Exception((msg is List) ? msg.join(', ') : (msg ?? 'Gagal'));
    }

    final data = body['data'];
    if (data is List) {
      return data
          .map((item) => ImunisasiModel.fromJson(Map<String, dynamic>.from(item as Map)))
          .toList();
    }
    return [];
  }

  // 2. GET JADWAL IMUNISASI BY ANAK ID
  Future<List<ImunisasiModel>> getJadwalImunisasiByAnakId(int anakId) async {
    final uri = Uri.parse('${ApiConstants.baseUrl}/ibu/jadwal-imunisasi/anak/$anakId');
    final response = await _client.get(uri, headers: _headers).timeout(const Duration(seconds: 15));
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) {
      final msg = body['message'];
      throw Exception((msg is List) ? msg.join(', ') : (msg ?? 'Gagal mengambil jadwal'));
    }
  ...
  Future<ImunisasiDetailModel> getJadwalImunisasiById(int id) async {
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) {
      final msg = body['message'];
      throw Exception((msg is List) ? msg.join(', ') : (msg ?? 'Gagal mengambil detail jadwal'));
    }
  ...
  Future<void> requestPerubahanJadwal(int jadwalId, String tanggalBaru, String alasan) async {
    ...
    final response = await _client.post(uri, headers: _headers, body: body).timeout(const Duration(seconds: 10));
    final decoded = jsonDecode(response.body);

    if (response.statusCode < 200 || response.statusCode >= 300) {
      final msg = decoded['message'];
      throw Exception((msg is List) ? msg.join(', ') : (msg ?? 'Gagal mengirim request'));
    }
  }
  ...
  Future<void> setJadwalSelesai(int jadwalId) async {
    final uri = Uri.parse('${ApiConstants.baseUrl}/ibu/jadwal-imunisasi/$jadwalId/selesai');
    final response = await _client.put(uri, headers: _headers).timeout(const Duration(seconds: 10));
    final body = jsonDecode(response.body);

    if (response.statusCode < 200 || response.statusCode >= 300) {
      final msg = body['message'];
      throw Exception((msg is List) ? msg.join(', ') : (msg ?? 'Gagal menyelesaikan jadwal'));
    }
  }
